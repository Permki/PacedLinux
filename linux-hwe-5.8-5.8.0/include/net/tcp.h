...
struct tcp_congestion_ops
{
	...
	u32 (*pace_offload)(const struct sock *sk, struct sk_buff *skb);
...
static inline struct tcp_fastopen_context *tcp_fastopen_get_ctx(const struct sock *sk)
{
...
static inline bool tcp_fastopen_cookie_match(const struct tcp_fastopen_cookie *foc,
...
static inline int tcp_fastopen_context_len(const struct tcp_fastopen_context *ctx)
{
	return ctx->num;
}
...
static inline u64 tcp_transmit_time(const struct sock *sk)
{
	if (static_branch_unlikely(&tcp_tx_delay_enabled))
	{
		u32 delay = (sk->sk_state == TCP_TIME_WAIT) ? tcp_twsk(sk)->tw_tx_delay : tcp_sk(sk)->tcp_tx_delay;
