                ...
                size_t size )
{
    ...
    //if (sizeof(pkt_rxed_in.pkt_hdr) < 20) 
 //   timing_loop();
    return pkt_hdr;
}
...
void timing_loop(){
  ...
  long period = 1000000;// local_csr_read(local_csr_mailbox0); // contains period in cycles and populated from host, use hwinfo to determine the right value to populate. Change this to contain delay retrieved from TCP-option
