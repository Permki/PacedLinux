...
/**
 * Returns the default bus for locating FIS data files
 *
 * @param dev           NFP interface
 */
NFP_API
int nfp_flash_default_spi_bus(struct nfp_device *dev);
 ...
 * @param length Length of image
 */
int nfp_flash_write_flash_one(struct nfp_device *nfp, void *buff, uint32_t length, int save, nfp_flash_progress_callback progress);
