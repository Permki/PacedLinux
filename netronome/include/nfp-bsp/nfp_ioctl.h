...
#define _IO(x, y)    CTL_CODE(0x8000 + (x), 0x800 + (y), \
			     METHOD_BUFFERED, FILE_ANY_ACCESS)
#define _IOR(x, y, t) CTL_CODE(0x8000 + (x), 0x800 + (y), \
			     METHOD_BUFFERED, FILE_READ_DATA)
#define _IOW(x, y, t) CTL_CODE(0x8000 + (x), 0x800 + (y), \
 ...
 */
#define NFP_IOCTL_CPP_AREA_REQUEST	_IOWR(NFP_IOCTL, 0x80, struct nfp_cpp_area_request)
 ...
 */
#define NFP_IOCTL_CPP_AREA_RELEASE	_IOW(NFP_IOCTL, 0x81, struct nfp_cpp_area_request)
 ...
 */
#define NFP_IOCTL_CPP_AREA_RELEASE_OBSOLETE	_IOW(NFP_IOCTL, 0x81, unsigned long)
...
struct nfp_cpp_explicit_request {
	...
	int in, out;		/* in and out data length (in 32-bit words) */
 ...
 */
#define NFP_IOCTL_CPP_EXPL_REQUEST	_IOW(NFP_IOCTL, 0x82, struct nfp_cpp_explicit_request)
...
struct nfp_cpp_identification {
	...
	uint32_t model;		/* NFP CPP model ID - see nfp_cpp_model() */
	uint32_t interface;		/* NFP CPP interface ID  - see nfp_cpp_interface() */
	uint32_t serial_lo;		/* Lower 32 bits of the 48 bit serial number */
	uint32_t serial_hi;		/* Upper 16 bits of the 48 bit serial number */
};
 ...
 */
#define NFP_IOCTL_CPP_EVENT_ACQUIRE     _IOW(NFP_IOCTL, 0x83, struct nfp_cpp_event_request)

/**
 * Release a NFP event that was bound to a signal
 */
#define NFP_IOCTL_CPP_EVENT_RELEASE     _IOW(NFP_IOCTL, 0x84, struct nfp_cpp_event_request)
 ...
 */
#define NFP_IOCTL_FIRMWARE_LOAD         _IOW(NFP_IOCTL, 0x8d, char[NFP_FIRMWARE_MAX])

/**
 * Get the name of the last firmware load attempt
 */
#define NFP_IOCTL_FIRMWARE_LAST         _IOR(NFP_IOCTL, 0x8e, char[NFP_FIRMWARE_MAX])
