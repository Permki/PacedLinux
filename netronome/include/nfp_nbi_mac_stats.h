...
struct nfp_nbi_mac_stats_state {
	...
	uint32_t ready;
	/**
	 * Counter incremented every cycle after the daemon completes a scan and
	 * update.
	 */
	uint64_t updated;
	/**
 ...
 * daemon parameter controls initialization of a mutex and should be set to 1
 * when called from the daemon and to 0 when called by a client.
 *
 ...
 */
NFP_NBI_API
int nfp_nbi_mac_stats_read_port(struct nfp_nbi_dev *nbi,
				int port,
 ...
 */
NFP_NBI_API
int nfp_nbi_mac_stats_read_chan(struct nfp_nbi_dev *nbi,
				int chan,
 ...
 */
NFP_NBI_API
int nfp_nbi_mac_stats_read_ilks(struct nfp_nbi_dev *nbi,
				int core,
