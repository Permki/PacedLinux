 ...
 *
 * @file          apps/managed_c/config.h
 * @brief         Infrastructure configuration for managed C application.
 */
 ...
 */
#define MAC_CHAN_PER_PORT       1
#define TMQ_PER_PORT            (8 * MAC_CHAN_PER_PORT)  // TM Q 8:1 fan in

#define PORT_TO_CHANNEL(x)      ((x) / MAC_CHAN_PER_PORT)
#define CHANNEL_TO_TMQ(y)       ((y) * TMQ_PER_PORT)
...
#define NBI_TM_CHANNEL_LEVEL_SELECT     0 /* MAC channel num defined to be
                                           * the level 0 scheduler ports */
#define NBI_TM_L1_INPUT_SELECT          0 /* Level 1 schedulers disabled */
#define NBI_TM_ENABLE_SHAPER            0 /* Rate limiting shaper disabled */

/* Configure number of TM queues */
#define NBI_TM_H_0_Q        ((MAC0_CHANNELS * TMQ_PER_PORT) + TMQ_PER_PORT)
...
/* ME pool per CTM */
#define ME_CTM_ALLOC_MAX_PKT_CREDITS    (TOTAL_CTM_PACKET_CREDITS/2)
#define ME_CTM_ALLOC_MAX_BUF_CREDITS    (TOTAL_CTM_BUFFER_CREDITS/2)
...
#define NUM_REDUCED_WORKER_ME           8       // i36 has less worker MEs
    ...
    #undef NBI_PACKET_CREDITS

    #define TOTAL_PACKET_CREDITS 1024

    /* NBI seqr is used. Only 1024 packets allowed in the system at one time */
    #define TOTAL_WORKER_MES    (((NUM_WORKER_ISLANDS-1) * NUM_WORKER_ME) \
                                 + NUM_REDUCED_WORKER_ME)

    #define NBI_PACKET_CREDITS  ((TOTAL_PACKET_CREDITS * NUM_WORKER_ME) \
                                / TOTAL_WORKER_MES)

    #define NBI_PACKET_CREDITS_REDUCED ((TOTAL_PACKET_CREDITS \
                                        * NUM_REDUCED_WORKER_ME) \
                                        / TOTAL_WORKER_MES)
#endif
...
/*
 * MAC port paused configuration - see SDN-1253
 */
#define PKTIO_MAC_FLUSH_ENABLED // Configure MAC flush on worker islands
