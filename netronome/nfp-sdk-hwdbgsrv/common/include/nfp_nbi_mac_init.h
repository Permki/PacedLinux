 ...
 *        hold=2 return with resets released. Link state should be checked later.
 * @return 0 on success
 * @return -1 on failure
 */
NFP_NBI_API
int nfp_nbi_mac_init(struct nfp_nbi_dev *nbi, struct nfp_nbi_mac_config *mcfg,
                     const struct nfp_nbi_phy_config *phy_config, int corepair, int hold);

NFP_NBI_API
int nfp_nbi_mac_phy_minimal_init(struct nfp_nbi_dev *nbi, struct nfp_nbi_mac_config *mcfg, const struct nfp_nbi_phy_config *phy_config);
