...
static int _encode_da(uint64_t *addr, int iid_lsb, int dest_island)
{
	uint64_t _u64 = _nic_mask64(iid_lsb + 5, iid_lsb, 0);
...
static int _encode_non_da(uint64_t *addr, int *isld, int idx_lsb, int iid_lsb, int vcnt,
		   int dest_island)
{
	int i, v;
 ...
 */
int _nfp6000_encode_basic(uint64_t *addr,
					    int dest_island,
					    int cpp_tgt,
					    int mode,
					    int addr40,
					    ...
					    int isld0)
{
	...
	switch (cpp_tgt) {
	case NFP6000_CPPTGT_MU:    /* This function doesn't handle MU */
	case NFP6000_CPPTGT_CTXPB: /* Also not CTXPB */
	...
	case 0:
		...
		return _encode_da(addr, addr40 ? 34 : 26, dest_island);
	case 1:
		if ((cpp_tgt == NFP6000_CPPTGT_VQDR) && !addr40) {
			i = _nfp6000_decode_basic(*addr, &v, cpp_tgt, mode, addr40, isld1,
						  isld0);
	...
	case 2:
		...
		return _encode_non_da(addr, isld, idx_lsb, iid_lsb, 2, dest_island);
	case 3:
		if ((cpp_tgt == NFP6000_CPPTGT_VQDR) && !addr40) {
			/* iid<0> = addr<29> = data */
		...
		return _encode_non_da(addr, isld, idx_lsb, iid_lsb, 4, dest_island);
	default:
		break;
	}
...
int _nfp6000_decode_basic(uint64_t addr,
					    int *dest_island,
					    int cpp_tgt,
					    int mode,
					    int addr40,
...
int _nfp6000_encode_mu(uint64_t *addr,
					 int dest_island,
					 int mode,
					 int addr40,
					 int isld1,
					 int isld0)
{
	...
	if (!mode || (mode < 3 && da))
		return _encode_da(addr, addr40 ? 32 : 24, dest_island);
	...
	case 1:
		...
		return NFP_ERRNO(ENODEV);
	case 2:
		/* Make sure compare against isldN values by clearing the LSB. */
		/* This is what the silicon does. */
		isld[0] &= ~1;
		isld[1] &= ~1;
...
int _nfp6000_decode_mu(uint64_t addr,
					 int *dest_island,
					 int mode,
					 int addr40,
					 int isld1,
...
int _nfp6000_cppat_addr_encode(uint64_t *addr,
						 int dest_island,
						 int cpp_tgt,
						 int mode,
						 int addr40,
...
int _nfp6000_cppat_addr_decode(uint64_t addr,
						 int *dest_island,
						 int cpp_tgt,
						 int mode,
						 int addr40,
...
int _nfp6000_cppat_addr_iid_clear(uint64_t *addr,
						    int cpp_tgt,
						    int mode,
						    int addr40)
{
